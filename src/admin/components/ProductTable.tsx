...
const ProductTable = ({
  ...
  const isSpecialized = isCosmetics || isArtisanal || isBazarChic || hideCategory;
      ...
      {viewMode === "grid" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {products.map((p) => {
            ...
            const isBazarChicItem = isBazarChic || catStr.includes("bazar") || catStr.includes("chic") || catStr.includes("antique");
            const isArtisanalItem = !isBazarChicItem && (isArtisanal || catStr.includes("artisan"));
            const isFull = (p.sale_mode ?? "decant") === "full_bottle" || isPack || isDeo || isBazarChicItem || isArtisanalItem || isCosmetics;
            ...
            return (
              <div
                key={p.id}
                className={`group relative bg-card border rounded-2xl p-3.5 sm:p-4 transition-all duration-300 hover:shadow-lg flex flex-col justify-between ${
                  isSelected
              ...
              >
                <div>
                  ...
                  <div className="relative aspect-square w-full rounded-xl overflow-hidden bg-muted/30 border border-border/50 mb-3">
                    ...
                    {onToggleSelect && (
                      <button
                        ...
                        title={isSelected ? "Désélectionner ce produit" : "Sélectionner ce produit"}
                      >
                        {isSelected ? (
                  ...
                  <p className="text-[10px] uppercase tracking-widest text-primary font-bold truncate">
                    {p.maison || (isArtisanalItem ? "Artisanat d'Excellence" : isBazarChicItem ? "Bazar Chic & Déco" : "Maison Kenzi")}
                  </p>
                  <h3 className="font-serif text-sm font-bold text-foreground truncate mt-0.5" title={p.name}>
                    {p.name}
                  </h3>

                  {/* Badges Spécifiques par Univers */}
                  {!isSpecialized && (
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                    {isCosmetics ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[#C9A96E]/15 text-[#C9A96E] border border-[#C9A96E]/30 truncate max-w-full">
                        <Flower2 className="w-3 h-3 shrink-0" />
                      ...
                      <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                        <Sparkles className="w-3 h-3" /> Bazar Chic & Déco
                      </span>
                    ) : isArtisanalItem ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                        <Palette className="w-3 h-3" /> Pièce Artisanale
                      </span>
                    ) : (
                      <>
                        {isPack && (
                ...
                <div className="mt-4 pt-3 border-t border-border/60 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={`font-semibold ${stockTotal === 0 ? "text-red-500" : "text-foreground"}`}>
                      {stockTotal} {isCosmetics ? "unités" : (isArtisanalItem || isBazarChicItem) ? "pièces" : isFull ? "unités" : "flacons"}
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-card border border-border/80 rounded-2xl overflow-hidden shadow-xs">
          <div className="overflow-x-auto w-full">
            <table className="w-full text-sm min-w-[680px] sm:min-w-[760px] border-collapse">
              <thead className="bg-muted/40 text-muted-foreground text-xs uppercase tracking-wide border-b border-border/80">
                <tr>
                  <th className="w-10 text-center px-2 py-3.5 shrink-0">
                    {onSelectAll && (
                      <button
                        type="button"
                        onClick={onSelectAll}
                  ...
                  <th className="w-10 text-left px-2 py-3.5 font-bold shrink-0 hidden sm:table-cell">#</th>
                  <th className="min-w-[200px] sm:min-w-[240px] text-left px-3.5 py-3.5 font-bold">
                    <button
                      type="button"
                  ...
                  <th className="min-w-[120px] text-left px-3.5 py-3.5 font-bold">
                    <button
                      type="button"
                      onClick={() => onSortChange && onSortChange("maison")}
                      className="inline-flex items-center gap-1 hover:text-foreground cursor-pointer"
                    >
                      <span>{isCosmetics ? "Marque" : isArtisanal ? "Atelier / Origine" : isBazarChic ? "Origine / Type" : "Maison"}</span>
                      <ArrowUpDown className="w-3 h-3" />
                    </button>
                  </th>
                  {!isSpecialized && (
                    <th className="min-w-[130px] text-left px-3.5 py-3.5 font-bold">
                      <button
                        type="button"
                  ...
                  <th className="min-w-[100px] text-right px-3.5 py-3.5 font-bold">
                    <button
                      type="button"
                  ...
                  <th className="min-w-[90px] text-right px-3.5 py-3.5 font-bold">
                    <button
                      type="button"
                  ...
                  <th className="min-w-[90px] text-center px-3.5 py-3.5 font-bold">
                    <button
                      type="button"
                  ...
                  <th className="min-w-[100px] text-right px-3.5 py-3.5 font-bold">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/60">
                {products.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="text-center px-2 py-3 w-10 shrink-0">
                        {onToggleSelect && (
                          <button
                            type="button"
                      ...
                      <td className="px-2 py-3 text-muted-foreground font-mono text-xs w-10 shrink-0 hidden sm:table-cell">{i + 1}</td>
                      <td className="px-3.5 py-3 font-medium text-foreground">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            <div
                              className="font-serif font-bold text-xs sm:text-sm leading-snug truncate block text-foreground max-w-[180px] sm:max-w-[280px]"
                              title={p.name}
                            >
                              {p.name}
                            </div>
                      ...
                      <td className="px-3.5 py-3 text-muted-foreground text-xs font-semibold truncate max-w-[130px]" title={p.maison}>
                        {p.maison || (isArtisanalItem ? "Artisanat" : isBazarChicItem ? "Bazar Chic" : "Maison Kenzi")}
                      </td>
                      {!isSpecialized && (
                        <td className="px-3.5 py-3">
                          <div className="flex flex-col gap-1 items-start truncate max-w-[140px]">
                      ...
                      <td className="px-3.5 py-3 text-right font-medium text-foreground whitespace-nowrap">
                        {isFull ? (
                          <span className="font-bold tracking-tight text-primary text-sm">{fmt(p.full_bottle_price ?? p.prices?.["5ml"] ?? 0)}</span>
                        ) : (
                          <div className="text-xs">
                            <div className="font-bold tracking-tight text-primary">{fmt(p.prices?.["5ml"] ?? 0)} <span className="text-[10px] text-muted-foreground font-normal">(5ml)</span></div>
                            <div className="text-muted-foreground text-[11px] font-medium tracking-tight">{fmt(p.prices?.["10ml"] ?? 0)} (10ml)</div>
                          </div>
                        )}
                      </td>
                ...
                {products.length === 0 && (
                  <tr>
                    <td colSpan={isSpecialized ? 7 : 8} className="px-4 py-16 text-center text-muted-foreground">
                      <Package className="w-10 h-10 mx-auto mb-2 opacity-40 text-primary" />
